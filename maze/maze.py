...
class Maze:
    ...
    def isPerfect(self)->bool:
        ...
        if len(self.getEntrances()) != 1 or len(self.getExits()) != 1:
            return False

        entrance = self.getEntrances()[0]
        exit = self.getExits()[0]
        visited = set()
        queue = deque([entrance])

        while queue:
            current = queue.popleft()
            if current == exit and len(visited) < self.rowNum() * self.colNum():
                # If we've reached the exit but haven't visited all cells, it's not perfect
                return False
            for direction in [(-1, 0), (1, 0), (0, -1), (0, 1)]:  # Up, down, left, right
                neighbor = (current[0] + direction[0], current[1] + direction[1])
                if (0 <= neighbor[0] < self.rowNum() and
                        0 <= neighbor[1] < self.colNum() and
                        neighbor not in visited and
                        not self.hasWall(current, neighbor)):
                    queue.append(neighbor)
                    visited.add(neighbor)

        return len(visited) == self.rowNum() * self.colNum()
